...
if($kale_frontpage_large_post_show == 1) { 
    ...
    if(has_filter('wpml_object_id')) $post = get_post( apply_filters( 'wpml_object_id', $kale_frontpage_large_post, 'post', TRUE ) );
    else $post = get_post($kale_frontpage_large_post); 
    setup_postdata($post); 
    $kale_entry = 'full';
